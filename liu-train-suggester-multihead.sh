...
#  --use-cpu \
rm -fr sockeye-commit-suggester-liu-multihead-nobin
...
python3 -m sockeye.train \
  ...
  --num-layers 2:2 \
  --transformer-attention-heads 16:16 \
  --source liu-dataset-nobin-full/cleaned.train.diff \
  --target liu-dataset-nobin-full/cleaned.train.msg \
  --output sockeye-commit-suggester-liu-multihead-nobin \
  --validation-source liu-dataset-nobin-full/cleaned.valid.diff \
  --validation-target liu-dataset-nobin-full/cleaned.valid.msg
#  --use-cpu \
